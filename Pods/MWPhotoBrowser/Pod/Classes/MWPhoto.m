...
- (void)performLoadUnderlyingImageAndNotify {
    ...
    if (_image) {
        ...
        if ([[[_photoURL scheme] lowercaseString] isEqualToString:@"assets-library"]) {
            ...
            // isFileURL added by seaphy
            // Load from local file async
            [self _performLoadUnderlyingImageAndNotifyWithLocalURL:_photoURL];
...
- (void)_performLoadUnderlyingImageAndNotifyWithLocalURL:(NSURL *)url {
            ...
            @try {
                ...
                if (imageSource) {
                    ...
                    options = @{(id)kCGImageSourceCreateThumbnailWithTransform : (id)kCFBooleanTrue
                                ...
                                , (id)kCGImageSourceThumbnailMaxPixelSize : (id)[NSNumber numberWithFloat:[UIScreen mainScreen].bounds.size.width * [UIScreen mainScreen].scale]
                                };
                    
                    CGImageRef imageRef = CGImageSourceCreateThumbnailAtIndex(imageSource, 0, (__bridge CFDictionaryRef)options);
                    self.underlyingImage = [UIImage imageWithCGImage:imageRef];
